...
WASAPIBuffer::WASAPIBuffer(const size_t packetMaxSize, const uint16_t headLength, const uint16_t tailLength)
:
...
bool WASAPIBuffer::canWrite() const
{
	return (_bufferAvailability > 0);
}


bool WASAPIBuffer::canRead() const
{
	return (_bufferAvailability < _headLength);
}


WASAPIBuffer::Slot& WASAPIBuffer::nextAvailable()
{
	if (!canWrite())
	{
...
WASAPIBuffer::Slot& WASAPIBuffer::nextBuffered()
{
	if (!canRead())
	{
...
const WASAPIBuffer::Slot& WASAPIBuffer::prevBuffered(const uint16_t tailIndex) const
{
